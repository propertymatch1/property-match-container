...
export function useAuth() {
  ...
  return {
    ...
    error,
    // Utility methods
    hasRole: (role: "TENANT" | "LANDLORD") =>
      hasRole(session?.user as User | null, role),
    canAccessTenantRoutes: () =>
      canAccessTenantRoutes(session as Session | null),
    canAccessLandlordRoutes: () =>
      canAccessLandlordRoutes(session as Session | null),
    getRedirectPath: (isSignup?: boolean) =>
      getRedirectPath(session?.user as User | null, isSignup),
  };
}
