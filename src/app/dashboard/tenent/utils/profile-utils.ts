...
export const PROFILE_FIELDS: ProfileField[] = [
  ...
  { key: "industry", label: "Industry", weight: 3, isComplete: (v) => !!v },
  {
    ...
    weight: 1,
    isComplete: (v) => Array.isArray(v) && v.length > 0,
  },
  {
    key: "typcialCustomer",
  ...
  },
  {
    ...
    label: "Rent Budget",
    weight: 3,
    isComplete: (v) => typeof v === "number" && v > 0,
  },
  {
    key: "cityNext",
    label: "Target Cities",
...
export function calculateCompleteness(
  ...
  return {
    ...
    completedFields: PROFILE_FIELDS.filter((f) => f.isComplete(profile[f.key]))
      .length,
